...
async function main() {
  ...
  async function makeCustomer(name) {
    const res = await CustomerAgent.create({ name: `${tag} ${name}`, partner_type: 2, default_sales_flow: 'INVENTORY_SALE', price_mode: 'PRIVATE_PRICE', billing_calendar_type: 'SOLAR' }, admin);
