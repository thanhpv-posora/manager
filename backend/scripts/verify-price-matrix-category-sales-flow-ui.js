...
async function main() {
  ...
  try {
    ...
    async function makeCustomer(name, defaultFlow) {
      const res = await CustomerAgent.create({ name: `VERIFY PMCF ${name} ${Date.now()}-${Math.random().toString(36).slice(2,6)}`, partner_type: 0, default_sales_flow: defaultFlow }, user);
