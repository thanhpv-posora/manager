...
async function main() {
  ...
  try {
    ...
    const partnerRes = await CustomerAgent.create({ name: `VERIFY SPOB Supplier ${Date.now()}`, phone: `09${String(Date.now()).slice(-6)}${String(Math.floor(Math.random()*90)+10)}`, partner_type: 1 }, user);
