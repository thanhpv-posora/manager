...
class DebtInstallmentAgent {
  ...
  async addPayment(planId,data,user){
    ...
    const plan=plans[0], amount=Number(data.amount||plan.daily_amount||0);
    if(amount<=0) throw new Error('Số tiền thu phải lớn hơn 0');
    const conn=await pool.getConnection();
    try{
      await conn.beginTransaction();
      const [r]=await conn.query(`INSERT INTO payments(customer_id,order_id,payment_date,amount,payment_method,note,created_by) VALUES(?,NULL,?,?,?,?,?)`,
        [plan.customer_id,data.payment_date,amount,data.payment_method||'CASH',data.note||`Thu góp công nợ #${planId}`,user?.id||null]);
      ...
      await conn.query(`INSERT INTO debt_installment_payments(plan_id,customer_id,payment_date,amount,payment_method,note,created_by) VALUES(?,?,?,?,?,?,?)`,
        [planId,plan.customer_id,data.payment_date,amount,data.payment_method||'CASH',data.note||'',user?.id||null]);
