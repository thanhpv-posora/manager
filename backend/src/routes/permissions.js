...
router.get('/my/menu-preferences',auth(['ADMIN','STAFF','CUSTOMER']),async(req,res,next)=>{try{res.json(await UserPermissionAgent.myMenuPreferences(req.user))}catch(e){next(e)}});
router.put('/my/menu-preferences',auth(['ADMIN','STAFF','CUSTOMER']),async(req,res,next)=>{try{res.json(await UserPermissionAgent.saveMyMenuPreferences(req.user,req.body.items))}catch(e){next(e)}});
router.delete('/my/menu-preferences',auth(['ADMIN','STAFF','CUSTOMER']),async(req,res,next)=>{try{res.json(await UserPermissionAgent.resetMyMenuPreferences(req.user))}catch(e){next(e)}});
