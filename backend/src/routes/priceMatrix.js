...
router.put('/:customerId', auth(['ADMIN','STAFF','CUSTOMER']), async(req,res,next)=>{try{res.json(await PriceMatrixAgent.saveMatrix(req.params.customerId, req.body.items, req.user.id,req.user))}catch(e){next(e)}});
...
router.post('/:customerId/catalog', auth(['ADMIN','STAFF','CUSTOMER']), async(req,res,next)=>{try{
  ...
  const own=productOwnerWhere(req.user,'p');
  if(own.sql){
    const [ok]=await pool.query(`SELECT p.id FROM products p WHERE p.id=? ${own.sql}`,[req.body.product_id,...own.params]);
    if(!ok.length) return res.status(403).json({message:'Không có quyền dùng mặt hàng này'});
  }
