...
export default function CreateOrder(){
  ...
  const currentCustomerLabel=currentCustomer
    ? `${currentCustomer.name} - ${currentCustomer.price_mode||''}`
    : 'Chưa chọn khách';
  ...
  useEffect(()=>{
    if(!cid)return;
    const business=isBusinessCustomer(currentCustomer);
    setBillCalendarType(business?'SOLAR':'LUNAR');
    if(!business&&!billLunarDateText){
  ...
  const addQuickProduct=async()=>{
    ...
    const r=await api.post('/products/quick',{
      ...quick,
      customer_id:cid,
      private_price:quick.sale_price
    });
  ...
  const applyVoiceCommand=(text)=>{
    ...
    const result=parseVoiceBillCommand(text,items);
    if(!result.ok){
    ...
    <SafePage loading={loading} error={error}>
      <div className="pos-agent-shell pos-real-shell">
        <POSHeaderAgent
          ...
          setCalendarType={setBillCalendarType}
          lunarDateText={billLunarDateText}
          setLunarDateText={setBillLunarDateText}
          dateOpen={dateOpen}
          ...
          <main className="pos-agent-main pos-real-main">
            <div className="card pos-customer-card pos-customer-collapse-card">
              ...
              <select className="select" value={cid} onChange={e=>loadCustomerCatalog(e.target.value)}>
                ...
                {customers.map(c=><option key={c.id} value={c.id}>{c.name} - {c.price_mode}</option>)}
              </select>
              ...
              {quickOpen&&(
                <div className="card inner-card">
                  ...
                  <div className="form-grid">
                    ...
                    <MoneyInput placeholder="Giá riêng cho khách này" value={quick.sale_price||0} onChange={v=>setQuick({...quick,sale_price:v})}/>
              ...
              {voiceOpen&&(
                <div className="card inner-card">
                  ...
                  <div className="actions">
                    ...
                    <input className="input" style={{maxWidth:360}} placeholder="Raw text giọng nói / gõ câu để test" value={voiceText} onChange={e=>setVoiceText(e.target.value)}/>
