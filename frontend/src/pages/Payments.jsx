...
export default function Payments(){
 ...
 const[overpayDialog,setOverpayDialog]=useState({open:false,selected:{},surplus:0,oldBills:[]});
 ...
 const doSave=async(allocateIds=[])=>{
  const res=await api.post('/payments',buildPayload(allocateIds));
  ...
  setOverpayDialog({open:false,selected:{},surplus:0,oldBills:[]});
  await loadSummary(form.customer_id);
  await loadPayments();
  let msg='Đã lưu thu tiền';
  if(allocs.length) msg += `\nĐã trừ nợ cũ: ${allocs.map(a=>`${a.order_code}: ${money(a.applied_amount)}`).join(', ')}`;
  if(unused>0) msg += `\nTiền dư chưa phân bổ: ${money(unused)}`;
  alert(msg);
 };
 const save=async()=>{
  const surplus=paidTotal-billTotal;
  const bills=oldDebtBills();
  // Khi khách đưa tiền lớn hơn bill đang thu, bắt buộc hiện dialog để chọn bill nợ cũ.
  // Không tự trừ âm thầm để tránh trừ nhầm bill.
  if(form.customer_id && paidTotal>0 && surplus>0 && bills.length){
   const selected={};
   let remain=surplus;
   for(const b of bills){
    if(remain<=0)break;
    selected[b.id]=true;
    remain-=Number(b.debt_amount||0);
   }
   setOverpayDialog({open:true,selected,surplus,oldBills:bills});
   return;
  }
  await doSave([]);
 };
 const confirmOverpay=async()=>{
  const ids=Object.entries(overpayDialog.selected||{}).filter(([,v])=>v).map(([k])=>Number(k));
  await doSave(ids);
 };
 const selectedOldDebtTotal=useMemo(()=>{
  return (overpayDialog.oldBills||[]).reduce((sum,b)=>sum+((overpayDialog.selected||{})[b.id]?Number(b.debt_amount||0):0),0);
 },[overpayDialog]);
 const toggleOldBill=id=>setOverpayDialog(d=>({...d,selected:{...(d.selected||{}),[id]:!(d.selected||{})[id]}}));
  ...
  {overpayDialog.open&&<div className="payment-overpay-overlay">
   <div className="card payment-overpay-dialog">
    <div className="payment-overpay-head">
     <div><h3>Khách đưa dư tiền - chọn bill nợ cũ để trừ</h3><p className="muted">Tiền khách đưa lớn hơn tổng bill đang thu. Chọn các bill còn nợ muốn trừ bằng phần tiền dư.</p></div>
     <button className="btn secondary" onClick={()=>setOverpayDialog({open:false,selected:{},surplus:0,oldBills:[]})}>Đóng</button>
    </div>
    <div className="payment-overpay-summary">
     <span>Bill đang thu: <b>{money(billTotal)}</b></span>
     <span>Khách đưa: <b>{money(paidTotal)}</b></span>
     <span>Tiền dư: <b>{money(overpayDialog.surplus)}</b></span>
     <span>Đã chọn nợ cũ: <b>{money(Math.min(selectedOldDebtTotal,overpayDialog.surplus))}</b></span>
    </div>
    <div className="payment-overpay-table-wrap">
     <table className="table payment-overpay-table"><thead><tr><th>Chọn</th><th>Bill</th><th>Ngày</th><th>Còn nợ</th></tr></thead><tbody>{overpayDialog.oldBills.map(o=><tr key={o.id}>
      <td><input type="checkbox" checked={!!overpayDialog.selected[o.id]} onChange={()=>toggleOldBill(o.id)}/></td>
      <td>{o.order_code}</td><td>{billDateLabel(o)}</td><td><b>{money(o.debt_amount)}</b></td>
     </tr>)}</tbody></table>
    </div>
    <div className="actions" style={{justifyContent:'space-between',marginTop:12}}>
     <button className="btn secondary" onClick={()=>doSave([])}>Không trừ nợ cũ, lưu tiền dư</button>
     <button className="btn" onClick={confirmOverpay}>Lưu và trừ bill đã chọn</button>
    </div>
   </div>
  </div>}
 </SafePage>;
