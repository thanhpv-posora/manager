...
export default function Payments(){
 ...
 const load=async()=>{try{const user=JSON.parse(localStorage.getItem('user')||'{}');await loadPayments();const cs=(await api.get('/partners',{params:{role:'customer'}})).data||[];setCustomers(cs);if(user.role!=='CUSTOMER'){if(!form.customer_id&&cs.length){setForm(f=>({...f,customer_id:String(cs[0].id)}));await loadSummary(cs[0].id)}}else if(user.customer_id){setForm(f=>({...f,customer_id:String(user.customer_id)}));await loadSummary(user.customer_id)}}catch(e){setError(e.response?.data?.message||e.message)}finally{setLoading(false)}};
 useEffect(()=>{load()},[]);
