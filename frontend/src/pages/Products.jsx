...
// Issue 4 (POS Import + Product Management UX hotfix): business-flow display
// mapping for the product list's "Luồng bán" column. Labels match the
// existing MeatBiz terminology already used elsewhere (e.g. the Luồng bán
// select just below, and CreateOrder.jsx's flowLabel()) — "Bò xô"/"Hàng kho",
// never the shorter "Xô"/"Kho". Deliberately keyed off sales_flow only — never
// derived from inventory_mode — per the hotfix's explicit instruction not to
// infer one from the other.
const SALES_FLOW_DISPLAY_LABELS={CARCASS_POS:'Bò xô',INVENTORY_SALE:'Hàng kho'};
...
export default function Products(){
  ...
  // Issue 3 (POS Import + Product Management UX hotfix): the Add/Edit form
  // moves into this Dialog. Same form/editing state as before, unchanged —
  // this flag only controls the Dialog's visibility.
  ...
  const save=async()=>{
    ...
    try{
      ...
      reset();
      // Issue 3: Dialog closes ONLY on successful save — the catch branch above
      // never reaches this line, so a failed save leaves productDialogOpen true,
      // the Dialog open, and (since reset() above is also skipped on failure)
  ...
  // Issue 3: single set of Dialog open/close handlers, reused by both Add and
  // Edit entry points. openAddDialog/openEditDialog only decide which form
  // state to load (reset() vs edit(x), both unchanged) before opening.
  // closeProductDialog is the one Cancel/Esc/backdrop/X path — it always calls
  // reset() first, so stale edit state can never leak into the next open.
  const openAddDialog=()=>{
    reset();
  ...
  return <SafePage loading={loading} error={error}>
    ...
    {/* Master Data Dialog Design System: header/body/footer standardized via
        Dialog's primaryAction/secondaryAction props (see Products.jsx report
        section "Làm mới" audit for why it moved into the body instead of the
        footer — its behavior in Edit mode is flagged REQUIRES_CTO_DECISION,
        not changed). All form/save/reset/edit/changeSalesFlow/category-dialog
        logic is unchanged — only the wrapper and footer construction changed.
        Dialog's built-in header "Đóng" button, Esc, and backdrop-click all
        already route through onClose=closeProductDialog, which resets stale
        edit state before the next open. */}
    <Dialog
      open={productDialogOpen}
      title={editing?'Sửa mặt hàng':'Thêm mặt hàng'}
      onClose={closeProductDialog}
      primaryAction={{label:editing?'Lưu sửa':'Thêm hàng',onClick:save}}
    >
      ...
      {/* CTO decision on the "Làm mới" audit finding: in Add mode this button
          is a genuine workflow convenience (same resulting state as
          Cancel+Reopen, but without leaving the dialog) — kept, visible only
          in Add mode. In Edit mode, reset() also clears `editing`, silently
          abandoning the edit into a blank Add form — this is a PRESENTATION
          GUARD only (hide the button while editing); reset() itself is
          completely unchanged, still does exactly what it did before, for
          any caller that still invokes it directly. */}
      {!editing && (
    ...
    <div className="card">
      ...
      <table className="table product-inline-table">
        {/* Issue 4: Mã (product_code) column removed from the visible list —
            the field itself is untouched everywhere else (data model, API,
            edit form via openEditDialog, client-side search in filteredRows
            above, audit). Replaced with Luồng bán, mapped from sales_flow
            only, never inferred from inventory_mode. */}
        <thead><tr><th>Tên</th><th>Danh mục</th><th>ĐVT</th><th>Giá bán</th><th>Luồng bán</th><th></th></tr></thead>
