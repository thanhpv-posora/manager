...
export default function Profit(){
 ...
 useEffect(()=>{load()},[]);
 const totals=useMemo(()=>rows.reduce((s,r)=>({revenue:s.revenue+Number(r.revenue||0),cost:s.cost+Number(r.cost||0),profit:s.profit+Number(r.profit||0),orders:s.orders+Number(r.orders||0),items:s.items+Number(r.items||0)}),{revenue:0,cost:0,profit:0,orders:0,items:0}),[rows]);
 ...
 return <SafePage loading={loading} error={error}><div className="grid">
  <div className="card"><h3>Thống kê lợi nhuận</h3><p className="muted">Doanh thu lấy theo <b>Ngày xuất hàng</b>. Giá vốn lấy theo <b>Ngày nhập hàng NCC</b> với hàng bò xô không kiểm tồn; hàng kiểm tồn dùng FIFO nếu có dữ liệu layer.</p><div className="actions"><input className="input" style={{width:180}} type="date" value={from} onChange={e=>setFrom(e.target.value)}/><input className="input" style={{width:180}} type="date" value={to} onChange={e=>setTo(e.target.value)}/><select className="select" style={{width:160}} value={group} onChange={e=>setGroup(e.target.value)}><option value="day">Theo ngày</option><option value="month">Theo tháng</option><option value="year">Theo năm</option></select><button className="btn" onClick={load}>Xem thống kê</button><button className="btn secondary" onClick={printProfit}>In thống kê</button></div></div>
  <div className="grid grid-4"><div className="stat-card"><span>Doanh thu</span><b>{money(totals.revenue)}</b></div><div className="stat-card"><span>Giá vốn</span><b>{money(totals.cost)}</b></div><div className="stat-card"><span>Lợi nhuận gộp</span><b>{money(totals.profit)}</b></div><div className="stat-card"><span>Tỷ suất LN</span><b>{pct(margin)}</b></div></div>
  <div className="card"><div style={{height:320}}><ResponsiveContainer width="100%" height="100%"><BarChart data={rows}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="period"/><YAxis/><Tooltip formatter={v=>money(v)}/><Bar dataKey="revenue"/><Bar dataKey="cost"/><Bar dataKey="profit"/></BarChart></ResponsiveContainer></div></div>
  <div className="card"><table className="table"><thead><tr><th>Kỳ</th><th>Doanh thu</th><th>Giá vốn</th><th>Lợi nhuận</th><th>Tỷ suất</th><th>Số bill</th></tr></thead><tbody>{rows.map((r,i)=><tr key={i}><td>{r.period}</td><td>{money(r.revenue)}</td><td>{money(r.cost)}</td><td><b>{money(r.profit)}</b></td><td>{pct(r.gross_margin)}</td><td>{r.orders}</td></tr>)}</tbody></table></div>
  <div className="card"><h3>Chi tiết giá vốn / lợi nhuận theo mặt hàng</h3><table className="table"><thead><tr><th>Ngày xuất</th><th>Bill</th><th>Khách</th><th>Mặt hàng</th><th>SL</th><th>Doanh thu</th><th>Giá vốn</th><th>Lãi</th><th>Cách tính</th></tr></thead><tbody>{details.slice(0,300).map((r,i)=><tr key={i}><td>{r.order_date}</td><td>{r.order_code}</td><td>{r.customer_name}</td><td>{r.product_name}</td><td>{Number(r.quantity||0).toLocaleString('en-US')}</td><td>{money(r.revenue)}</td><td>{money(r.cost)}</td><td>{money(r.profit)}</td><td>{r.cost_mode}</td></tr>)}</tbody></table>{details.length>300&&<p className="muted">Đang hiển thị 300 dòng đầu. Dùng lọc ngày để xem chi tiết nhỏ hơn.</p>}</div>
 </div></SafePage>
}
