...
export default function SponsorVideos(){
 ...
 const generateRealVideo=async()=>{try{const sponsor=sponsors.find(x=>String(x.id)===String(form.sponsor_id));setProgress(0);const blob=await generateSponsorVideo({...form,sponsor_name:sponsor?.name,durationSec:12,onProgress:setProgress});const url=URL.createObjectURL(blob);setPreviewUrl(url);await uploadBlob(blob);alert('Đã tạo và upload video')}catch(e){alert(e.message||'Không tạo được video trên trình duyệt này')}};
 ...
 return <SafePage loading={loading} error={error}>
  ...
  </div><div className="actions" style={{marginTop:12}}><button className="btn secondary" onClick={makeIdea}>AI tạo ý tưởng</button><button className="btn secondary" onClick={generateRealVideo}>Tạo video</button><button className="btn" onClick={createPlan}>Lưu & đưa vào danh sách</button></div>
  ...
  <div className="card"><h3>Agent AI tự học</h3><p className="muted">Khi sửa prompt/kịch bản và publish, hệ thống lưu learning log để lần sau tạo nội dung sát nghiệp vụ hơn.</p><p>Video tạo nội bộ là slideshow WebM. Nếu cần video AI người nói/cảnh quay thì upload file video từ công cụ ngoài vào đây.</p></div></div>
  <div className="card"><h3>Danh sách video đang dùng</h3><table className="table"><thead><tr><th>Video</th><th>Vị trí</th><th>Preview</th><th>Trạng thái</th><th></th></tr></thead><tbody>{videos.map(v=><tr key={v.id}><td><b>{v.title}</b><br/><span className="muted">{v.sponsor_name} · {v.campaign_date}</span></td><td>{placements.find(p=>p[0]===v.placement)?.[1]||v.placement}</td><td>{v.video_url?<video src={v.video_url} controls style={{width:220,borderRadius:12}}/>:'Chưa có video'}</td><td>{v.status} · {v.is_public?'Public':'Ẩn'}</td><td><div style={{display:'flex',flexWrap:'nowrap',gap:6,alignItems:'center',justifyContent:'center'}}><button className="btn secondary" title="Sửa" style={{padding:0,width:32,height:32,display:'inline-flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setForm({...v})}><Pencil size={14}/></button><button className={v.is_public?'btn secondary':'btn'} title={v.is_public?'Ẩn':'Publish'} style={{padding:0,width:32,height:32,display:'inline-flex',alignItems:'center',justifyContent:'center'}} onClick={()=>publish(v,!v.is_public)}>{v.is_public?<EyeOff size={14}/>:<Eye size={14}/>}</button><button className="btn danger" title="Xóa mềm" style={{padding:0,width:32,height:32,display:'inline-flex',alignItems:'center',justifyContent:'center'}} onClick={()=>softDelete(v)}><Trash2 size={14}/></button></div></td></tr>)}</tbody></table></div>
  <div className="card"><h3>Video đã xóa mềm</h3><table className="table"><tbody>{deleted.map(v=><tr key={v.id}><td>{v.title}<br/><span className="muted">{v.deleted_reason}</span></td><td><div style={{display:'flex',flexWrap:'nowrap',gap:6,alignItems:'center',justifyContent:'center'}}><button className="btn secondary" title="Khôi phục" style={{padding:0,width:32,height:32,display:'inline-flex',alignItems:'center',justifyContent:'center'}} onClick={()=>restore(v)}><RotateCcw size={14}/></button><button className="btn danger" title="Xóa vĩnh viễn" style={{padding:0,width:32,height:32,display:'inline-flex',alignItems:'center',justifyContent:'center'}} onClick={()=>hardDelete(v)}><Trash2 size={14}/></button></div></td></tr>)}</tbody></table></div>
 </SafePage>
}
