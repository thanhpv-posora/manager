...
router.put('/:customerId', auth(['ADMIN','STAFF','CUSTOMER']), async(req,res,next)=>{try{res.json(await PriceMatrixAgent.saveMatrix(req.params.customerId, req.body.items, req.user.id, req.body.effective_from || req.body.effectiveFrom || req.body.apply_date))}catch(e){next(e)}});
...
router.post('/copy', auth(['ADMIN','STAFF','CUSTOMER']), async(req,res,next)=>{try{res.json(await PriceMatrixAgent.copyCatalog(req.body.from_customer_id, req.body.to_customer_id, req.user.id, req.body.effective_from || req.body.effectiveFrom || req.body.apply_date))}catch(e){next(e)}});
