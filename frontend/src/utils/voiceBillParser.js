...
function parseCommand(text) {
  ...
  const productName = parts.slice(0,start).join(' ').trim();
...
function scoreProduct(spokenName, product) {
  const a = normalizeText(spokenName).replace(/\s/g,'');
  const b = normalizeText(product.product_name || product.name || '').replace(/\s/g,'');
